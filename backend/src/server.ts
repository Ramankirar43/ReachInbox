...
app.get('/api/emails', requireAuth, async (req, res) => { const requested = String(req.query.status ?? 'scheduled'); const status = ['scheduled', 'sent', 'failed'].includes(requested) ? requested : 'scheduled'; const result = await db.query('SELECT id,recipient,subject,scheduled_at,status,sent_at,error FROM emails WHERE user_id=$1 AND status=$2 ORDER BY COALESCE(sent_at,scheduled_at) DESC LIMIT 500', [(req.user as any).id, status]); res.json(result.rows); });
app.get('/api/emails/counts', requireAuth, async (req, res) => { const result = await db.query("SELECT status, COUNT(*)::int AS count FROM emails WHERE user_id=$1 AND status IN ('scheduled','sent','failed') GROUP BY status", [(req.user as any).id]); res.json(result.rows.reduce((counts, row) => ({ ...counts, [row.status]: row.count }), { scheduled: 0, sent: 0, failed: 0 })); });
